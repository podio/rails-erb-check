    ...
    stages {
        stage("Clone Git Repo") {
            steps {
                ...
                checkout([
                    ...
                    extensions: [
                        [$class: 'RelativeTargetDirectory', relativeTargetDir: 'rails-erb-check'],
                        [$class: 'CloneOption', shallow: true, depth: 1, noTags: false]
                    ],
                    submoduleCfg: [],
                    userRemoteConfigs: [[credentialsId: 'github-app-podio-jm', url: 'https://github.com/podio/rails-erb-check.git']]
                ])
            }
        }
        ...
        stage('Polaris') {
            environment {
                ...
                BRIDGE_POLARIS_PROJECT_NAME = "rails-erb-check"
        ...
        stage('BlackDuckSCA') {
            environment {
                ...
                BRIDGE_DETECT_ARGS = "--detect.project.name=DX-Podio-rails-erb-check --detect.project.version.name=${env.branchName} --detect.project.version.update=true --detect.project.version.distribution=SAAS --detect.project.group.name=Podio-Podio --detect.accuracy.required=NONE --detect.timeout=900"
            }
            steps {
                dir('rails-erb-check') {
                    script {
        ...
        stage('TruffleHog Scan') {
            steps {
                script {
                    ...
                    def repoPath = "${WORKSPACE}/rails-erb-check"
